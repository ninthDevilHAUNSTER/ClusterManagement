...
//	if (!xdebug_hash_index_find(file->lines, lineno, (void *) &line)) {
    ...
    if(XG(dupline)->lineno == line->lineno && (strcmp(XG(dupline)->filename, filename) == 0)){
        return;
    }

    XG(dupline)->lineno = line->lineno;
    XG(dupline)->filename = strdup(filename);

    fprintf(XG(fp), "%s,%d\n", filename, line->lineno);

//    int seek = strlen("/Users/lfy/Downloads/FDU/php-src/bin/test_php/");
//    strncpy(raw_file_name, filename+seek, strlen(filename)-seek);

//    fp = fopen("/Users/lfy/Downloads/FDU/result.txt", "a");
